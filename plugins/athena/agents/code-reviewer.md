...
When performing pre-landing review (from `/athena:ship` or explicit checklist request), load and apply checklists from `code-review/references/checklists/` using the workflow in `code-review/references/checklist-workflow.md`. Two-pass model: critical (blocking) + informational (non-blocking).
...
Use `/athena:scout` with edge-case-focused prompt:
