...
- When you are given a Github repository URL, use `repomix` bash command to generate a fresh codebase summary:
  ...
  ```
- You can use `/athena:scout ext` (preferred) or `/athena:scout` (fallback) slash command to search the codebase for files needed to complete the task
...
7. **Finalize Phase**: Ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/athena:plan --fast` or `/athena:plan --hard` slash command based on complexity.
