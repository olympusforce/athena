...
| Documentation | `.athena/docs/`                | Human & AI readable docs, guides, specs                               |
| Plans         | `.athena/plans/`               | Implementation plans, research, agent reports, chronological journals |
...
| Guides        | `.athena/guide/` or `.athena/guides/`  | User-facing reference docs, tutorials                                 |
...
**Date format:** Use default `YYMMDD-HHmm`.
...
| ------------------ | --------------------------- | -------------------------------------------------- |
| Single file output | Flat file in category dir   | `.athena/plans/journals/260304-session-review.md`          |
| Multi-file output  | Self-contained subdirectory | `.athena/plans/260304-auth-impl/plan.md` + `phase-01-*.md` |
| Scoped to parent   | Nested under parent context | `.athena/plans/260304-auth-impl/reports/scout-report.md`   |
...
When invoked directly with `/athena:project-organization [targets]`:
...
- `exec` / `fix` → source code paths (defer to language conventions)
