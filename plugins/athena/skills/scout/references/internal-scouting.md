# Internal Scouting with ghost-rider Subagents

Use `ghost-rider` subagents when SCALE >= 6 or external tools unavailable, and only
when the active runtime permits the `delegate_agent` capability.

## Delegation Gate

Do not spawn `ghost-rider` only because this reference says to use `ghost-rider`. Spawn
`ghost-rider` only when:
...
Spawn multiple `ghost-rider` subagents through the runtime's `delegate_agent`
...
```text
delegate_agent capability(subagent_type="athena:ghost-rider", prompt="<prompt>", description="<short scope>")
```
...
- `api/` - API routes
- `core/` - Core services
- `shared/` - Shared reusable services, utilities, ui, ...
