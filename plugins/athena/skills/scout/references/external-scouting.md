...
Antigravity requested           -> route through orchestrate
```

Ordinary scouting must not invoke Antigravity directly. The `orchestrate`
...
scoped `run_shell` searches. Use internal `ghost-rider` delegation only when the user
