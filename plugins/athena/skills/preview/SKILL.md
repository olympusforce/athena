...
- `/athena:preview <file.md>` - View markdown file in novel-reader UI
- `/athena:preview <directory/>` - Browse directory contents
- `/athena:preview --stop` - Stop running server

### Generation Mode (Markdown)

- `/athena:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/athena:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/athena:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/athena:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)

- `/athena:preview --html --explain <topic>` - Self-contained HTML explanation
- `/athena:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/athena:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/athena:preview --html --diff [ref]` - Visual diff review
- `/athena:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/athena:preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic                               | Ask user: "Please provide a topic: `/athena:preview --explain <topic>`"                       |
...
| Server startup failure                           | Check if port in use, try `/athena:preview --stop` first                                      |
...
Use or research relevant skill for Mermaid syntax validation.
