...
3. If no plan found → Ask user to specify path or run `/athena:plan` first
...
Launch reviewers simultaneously via delegate_agent capability with `subagent_type: "athena:code-reviewer"`.
...
Remind user they can run `/athena:plan validate {plan-directory-path}` before implementation.
When the user approves implementation, run `/athena:exec {ABSOLUTE_PATH_TO_PLAN_DIR}/plan.md`.
