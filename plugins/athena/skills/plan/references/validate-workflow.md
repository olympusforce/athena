...
3. If no plan found → Ask user to specify path or run `/athena:plan --hard` first
...
>
> ```
> /exec {ABSOLUTE_PATH_TO_PLAN_DIR}/plan.md
> ```
>
> **Flag selection:** If Verification Results show `Failed: 0`, the plan is eligible for implementation. Ask the user before proceeding. Add `--auto` only when the user explicitly asks for autonomous implementation. If `Failed: N > 0`, revise the plan before execution.
...
- Never recommend execution until the whole-plan consistency sweep has no unresolved contradictions
