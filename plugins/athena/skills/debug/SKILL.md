...
- **Scouting:** `/athena:scout` or `/athena:scout ext` for finding relevant files
...
**Typically follows:** `/athena:scout` (after locating relevant code)
**Typically precedes:** `/athena:fix` (fix the diagnosed issue), `/athena:brainstorm` (explore solutions for complex problems)
**Related:** `/athena:scout` (discover before debugging), `/athena:fix` (fix after diagnosing)
