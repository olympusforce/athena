...
import { myFeatureApi } from '../api/my-module.api';
...
export const useSuspensePost = (postId: number) => {
...
modules/
  my-module/
    api/
      my-module.api.ts    # Service layer
```

### Service Pattern (from post.api.ts)
...
- Use `apiClient` (axios instance from `~shared/api/api-client`)
...
```typescript
export const useEntityFromCache = (blogId: number, entityId: number) => {
  const queryClient = useQueryClient();
...
1. **Create API Service**: `modules/x/api/x.api.ts` using apiClient
