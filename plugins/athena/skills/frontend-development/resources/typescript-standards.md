...
// For custom hooks
const useMyData = (id: number): { data: Data; isLoading: boolean } => {
...
import { User } from '~entities/user/model/user.type'; // Unclear if type or value
```
...
const Component = ({ state }: { state: LoadingState }) => {
...
```typescript
const getById = <T,>(items: T[], id: number): T | undefined => {
...
export const List = <T,>({ items, renderItem }: ListProps<T>): React.ReactElement => {
    return (
        <div>
